#!/usr/bin/env python3
"""
CM Flexi Cap Fund — Daily Update
Fetches NAV + returns + AUM for CM Flexi Cap and all peer funds
from AMFI (category=1, subCategory=3).
Inception date: 2026-03-16

Usage:
    python flexi_daily.py
    python flexi_daily.py --full   (rebuild full history)
"""
...
INCEPTION_DATE = datetime.date(2024, 8, 4)
...
AMFI_PAYLOAD = {"maturityType": 1, "category": 3, "subCategory": 34, "mfid": 0}
...
# Scheme codes for direct plans — category 1/subCategory 3 (Flexi Cap Allocation)
# Capitalmind Flexi Cap is identified by "CAPITALMIND" in scheme name
...
def get_fund_label(scheme_name):
    """Extract AMC name — everything before 'Flexi' / 'Flexicap' in the scheme name."""
    import re
    if "CAPITALMIND" in scheme_name.upper():
        return "Capitalmind"
    m = re.search(r'\bFlexi\b|\bFlexicap\b', scheme_name, re.IGNORECASE)
    if m:
...
def ingest(full_history=False, override_t1=None):
    ...
    print(f"\n{'='*50}")
    print(f"  CM FLEXI CAP — {'Full Rebuild' if full_history else 'Daily Update'}")
    print(f"{'='*50}")
    ...
    # Exclude index funds not in the active peer universe
    EXCLUDED = {"dsp nifty500", "dsp nifty 500"}
    funds_data = [f for f in funds_data
                  if not any(ex in f["fund_label"].lower() for ex in EXCLUDED)]
