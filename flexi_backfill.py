...
"""Flexi Fund NAV Rankings Ingest — mirrors nav_ingest.py for flexi funds.
...
Periods:
    ...
    SI   = NAV on 06-Aug-2025 (nearest trading day), n = today - 06-Aug-2025
 
Run daily after update_liquid_nav.py and AUM_timeseries.py:
    python flexi_nav_ingest.py
"""
...
DB_PATH        = os.path.join(BASE_DIR, "flexi_rankings.db")
INCEPTION_DATE = datetime.date(2025, 8, 6)   # Capitalmind Flexi Cap fund inception
...
# Hardcoded scheme codes for all flexicap direct growth funds
...
SCHEME_CODES = {
    ...
    # 153801: "DSP Nifty500 Flexicap Quality 30 Index Fund - Direct - Growth",  # excluded — index fund, not active peer
    ...
    154225: "Old Bridge Flexi Cap Fund Direct Growth",
...
# -- Shorten SCHEME_CODES labels to AMC name (everything before "Flexi"/"Flexicap") --
import re as _re

def _shorten_flexi(name):
    if "CAPITALMIND" in name.upper():
        return "Capitalmind"
    m = _re.search(r"\bFlexi\b|\bFlexicap\b", name, _re.IGNORECASE)
    if m:
...
SCHEME_CODES = {k: _shorten_flexi(v) for k, v in SCHEME_CODES.items()}

# Set of known fund_labels for fast lookup
_KNOWN_LABELS = set(SCHEME_CODES.values())

# Capitalmind label -- must exactly match the SCHEME_CODES entry above
...
def fetch_nav_from_api(full_history=False):
    """Fetch NAV for all flexicap funds from AMFI portal.
    ...
    if not full_history:
        ...
        print("Fetching flexi NAV from AMFI NAVOpen.txt ...")
        rows = []
        ...
        print(f"Fetching full flexi NAV history from AMFI (back to {lookback_start})...")
...
def fetch_aum_from_api(category=1, subcategory=3):
    """Fetch latest AUM from AMFI for flexi cap funds.
    category=1 (Equity), subCategory=45 (Flexi Cap).
    ...
    print("Fetching flexi AUM from AMFI...")
    ...
    for report_date in dates_to_try:
        try:
            ...
            for _, row in df_raw.iterrows():
                ...
                aum      = row.get(col)
                label    = match_aum_label(raw_name)
                if label and aum:
                    try:
            ...
            if rows:
                ...
                print(f"  Flexi AUM fetched for {report_date}: {len(df)} funds")
...
def ingest(override_t1=None, full_history=False):
    ...
    for nav_date in new_dates:
        ...
        for _, row in day_nav.iterrows():
            ...
            # ── Since Inception ──
            nav_si, _ = get_nav_for_target(fund_ts, inception_ts, search_window=5)
    ...
    # Print clear summary — guarded against None when AUM was never fetched
    import sqlite3 as _sq2
    _c2 = _sq2.connect(DB_PATH)
    _nav_latest  = _c2.execute("SELECT MAX(nav_date) FROM nav_rankings WHERE nav IS NOT NULL").fetchone()[0]
    _aum_latest  = _c2.execute("SELECT MAX(nav_date) FROM nav_rankings WHERE aum_cr IS NOT NULL").fetchone()[0]
    _nav_count   = _c2.execute("SELECT COUNT(*) FROM nav_rankings WHERE nav_date=? AND nav IS NOT NULL", (_nav_latest,)).fetchone()[0] if _nav_latest else 0
    _aum_count   = _c2.execute("SELECT COUNT(*) FROM nav_rankings WHERE nav_date=? AND aum_cr IS NOT NULL", (_aum_latest,)).fetchone()[0] if _aum_latest else 0
    _c2.close()
    print(f"\n{'='*50}")
    print(f"  Done. DB: {DB_PATH}")
    print(f"  NAV updated : {_nav_count} funds (latest: {_nav_latest})")
    print(f"  AUM updated : {_aum_count} funds (latest: {_aum_latest or 'none'})")
    print(f"{'='*50}")
...
_AUM_PAYLOAD = {"maturityType": 1, "category": 1, "subCategory": 3, "mfid": 0}
    ...
    for attempt in range(1, 4):
        try:
            ...
            for _, row in df.iterrows():
                label = match_aum_label(str(row.get("schemeName", "")))
                aum   = row.get(col)
...
def _backfill_aum():
    ...
    print(f"\n{'='*60}")
    print(f"  Flexi AUM BACKFILL — {_AUM_START} to {date.today()}")
    print(f"{'='*60}")
    conn = sqlite3.connect(DB_PATH)
    ...
    with _req2.Session() as session:
        ...
        while d <= date.today():
            ...
            _time2.sleep(0.35)
    conn.close()
    print(f"\n  Done flexi AUM: updated={updated} inserted={inserted} empty={empty}")
...
if __name__ == "__main__":
    print("=" * 50)
    print("  Flexi Fund — Full Backfill")
    print("=" * 50)
