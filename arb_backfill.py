#!/usr/bin/env python3
"""Arbitrage Fund NAV Rankings Ingest — mirrors nav_ingest.py for arbitrage funds.
...
DB_PATH        = os.path.join(BASE_DIR, "arb_rankings.db")
INCEPTION_DATE = datetime.date(2026, 3, 16)   # Capitalmind ARB fund inception
...
# FUND_MAP and AUM_MAP are defined after SCHEME_CODES below
...
# Hardcoded scheme codes for all arbitrage direct growth funds
# (mfapi.in schemeCode → short display label; update if new funds are added)
SCHEME_CODES = {
    119526: "Aditya Birla Sun Life",
    130773: "Axis",
    152077: "Bajaj Finserv",
    118474: "Bandhan",
    143614: "Bank of India",
    150251: "Baroda BNP Paribas",
    154236: "Capitalmind",
    142283: "DSP",
    130206: "Edelweiss",
    153043: "Franklin Templeton",
    118931: "HDFC",
    151138: "HSBC",
    154257: "Helios",
    120364: "ICICI P",
    ...
    120482: "JM Financial",
    154076: "Jio B",
    119771: "Kotak Mahindra",
    145895: "LIC",
    148468: "Mahindra Manulife",
    148401: "Mirae Asset",
    153187: "Motilal Oswal",
    150367: "NJ",
    118585: "Nippon India",
    138875: "PGIM India",
    152109: "PPFAS",
    ...
    153804: "Trust",
    145724: "Tata",
    153891: "The Wealth Company",
    146297: "Union",
    120795: "UTI",
    152849: "WhiteOak Capital",
}

# AMFI schemeName → short display label (uppercase substring match)
# Keys must be uppercase substrings that uniquely identify each fund in the AMFI response
# for category=3, subCategory=33 (Arbitrage Fund)
FUND_MAP = {
    "ADITYA BIRLA SUN LIFE ARBITRAGE":    "Aditya Birla Sun Life",
    "AXIS ARBITRAGE":                      "Axis",
    "BAJAJ FINSERV ARBITRAGE":             "Bajaj Finserv",
    "BANDHAN ARBITRAGE":                   "Bandhan",
    "BANK OF INDIA ARBITRAGE":             "Bank of India",
    "BARODA BNP PARIBAS ARBITRAGE":        "Baroda BNP Paribas",
    "CAPITALMIND ARBITRAGE":               "Capitalmind",
    "DSP ARBITRAGE":                       "DSP",
    "EDELWEISS ARBITRAGE":                 "Edelweiss",
    "FRANKLIN INDIA ARBITRAGE":            "Franklin Templeton",
    "HDFC ARBITRAGE":                      "HDFC",
    "HSBC ARBITRAGE":                      "HSBC",
    "HELIOS ARBITRAGE":                    "Helios",
    "ICICI PRUDENTIAL EQUITY ARBITRAGE":   "ICICI P",
    "ITI ARBITRAGE":                       "ITI",
    "INVESCO INDIA ARBITRAGE":             "Invesco",
    "JM ARBITRAGE":                        "JM Financial",
    "JIOBLACKROCK ARBITRAGE":              "Jio B",
    "JIO BLACKROCK ARBITRAGE":             "Jio B",
    "KOTAK ARBITRAGE":                     "Kotak Mahindra",
    "LIC MF ARBITRAGE":                    "LIC",
    "MAHINDRA MANULIFE ARBITRAGE":         "Mahindra Manulife",
    "MIRAE ASSET ARBITRAGE":               "Mirae Asset",
    "MOTILAL OSWAL ARBITRAGE":             "Motilal Oswal",
    "NJ ARBITRAGE":                        "NJ",
    "NIPPON INDIA ARBITRAGE":              "Nippon India",
    "PGIM INDIA ARBITRAGE":                "PGIM India",
    "PARAG PARIKH ARBITRAGE":              "PPFAS",
    "QUANT ARBITRAGE":                     "quant",
    "SBI ARBITRAGE":                       "SBI",
    "SAMCO ARBITRAGE":                     "Samco",
    "SUNDARAM ARBITRAGE":                  "Sundaram",
    "TRUSTMF ARBITRAGE":                   "Trust",
    "TATA ARBITRAGE":                      "Tata",
    "THE WEALTH COMPANY ARBITRAGE":        "The Wealth Company",
    "UNION ARBITRAGE":                     "Union",
    "UTI ARBITRAGE":                       "UTI",
    "WHITEOAK CAPITAL ARBITRAGE":          "WhiteOak Capital",
}
...
def fetch_nav_from_api(full_history=False):
    """Fetch NAV for all arbitrage direct funds from AMFI portal.
    ...
    if not full_history:
        ...
        print("Fetching ARB NAV from AMFI NAVOpen.txt ...")
        ...
        print(f"Fetching full ARB NAV history from AMFI (back to {lookback_start})...")
...
def fetch_aum_from_api(category=3, subcategory=33):
    """Fetch latest AUM from AMFI for arbitrage funds.
    category=3 (Hybrid), subCategory=33 (Arbitrage).
    Uses preMonthAUM column — tries today and each prior day up to 10 days back.
    Cookies: _ga based — refresh from browser DevTools if response is empty.
    """
    import requests, datetime as dt
    print("Fetching ARB AUM from AMFI...")
    ...
    for report_date in dates_to_try:
        try:
            ...
            if rows:
                ...
                print(f"  ARB AUM fetched for {report_date}: {len(df)} funds")
                return df, report_date

        except Exception as e:
            print(f"  WARN: ARB AUM fetch for {report_date} failed: {e}")

    print("  WARNING: Could not fetch ARB AUM — rankings will show without AUM")
...
# AUM BACKFILL — inlined (ARB portion only, uses dailyAUM)
# ══════════════════════════════════════════════════════════════════════════════
...
_AUM_PAYLOAD = {"maturityType": 1, "category": 3, "subCategory": 33, "mfid": 0}

_ARB_AUM_MAP = {
    "ADITYA BIRLA": "Aditya Birla Sun Life", "AXIS ARB": "Axis",
    "BAJAJ FINSERV": "Bajaj Finserv", "BANDHAN": "Bandhan",
    "BANK OF INDIA": "Bank of India", "BARODA BNP": "Baroda BNP Paribas",
    "CAPITALMIND": "Capitalmind", "DSP ARB": "DSP", "EDELWEISS": "Edelweiss",
    "FRANKLIN": "Franklin Templeton", "HDFC ARB": "HDFC", "HELIOS": "Helios",
    "HSBC ARB": "HSBC", "ICICI PRUDENTIAL": "ICICI P", "INVESCO": "Invesco",
    "ITI ARB": "ITI", "JIOBLACKROCK": "Jio B", "JIO BLACKROCK": "Jio B",
    "JM ARB": "JM Financial", "KOTAK": "Kotak Mahindra", "LIC MF ARB": "LIC",
    "MAHINDRA": "Mahindra Manulife", "MIRAE": "Mirae Asset", "MOTILAL": "Motilal Oswal",
    "NIPPON": "Nippon India", "NJ ARB": "NJ", "OLD BRIDGE": "Old Bridge",
    "PARAG PARIKH": "PPFAS", "PGIM": "PGIM India", "QUANT ARB": "quant",
    "SAMCO": "Samco", "SBI ARB": "SBI", "SUNDARAM": "Sundaram",
    "TATA ARB": "Tata", "THE WEALTH COMPANY": "The Wealth Company",
    "TRUSTMF": "Trust", "UNION ARB": "Union", "UTI ARB": "UTI",
    "WHITEOAK": "WhiteOak Capital",
}

def _aum_map_name(raw):
    u = raw.upper()
    for key, label in _ARB_AUM_MAP.items():
        if key in u:
            return label
    return None
...
def _fetch_aum_date(report_date_str, session):
    ...
    for attempt in range(1, 4):
        try:
            r = session.post(_AUM_URL, cookies=_AUM_COOKIES,
                             headers=_AUM_HEADERS, json=payload, timeout=30)
            r.raise_for_status()
            data = r.json()
            items = data if isinstance(data, list) else data.get("data", [])
            if not items: return {}
            ...
            if not col or "schemeName" not in df.columns: return {}
            result = {}
            for _, row in df.iterrows():
                label = _aum_map_name(str(row.get("schemeName", "")))
                aum = row.get(col)
                if label and aum is not None:
                    try:
                        val = float(aum)
                        if val > 0: result[label] = val
...
def _backfill_arb_aum():
    from datetime import date, timedelta
    print(f"\n{'='*60}")
    print(f"  ARB AUM BACKFILL — {_AUM_START} to {date.today()}")
    print(f"{'='*60}")
    conn = sqlite3.connect(DB_PATH)
    ...
    with _req2.Session() as session:
        ...
        while d <= date.today():
            ...
            if not aum_map:
                empty += 1; d += timedelta(days=1); _time2.sleep(0.3); continue
            for label, aum_cr in aum_map.items():
                row = conn.execute("SELECT id FROM nav_rankings WHERE nav_date=? AND fund_label=?", (date_str, label)).fetchone()
                if row:
                    conn.execute("UPDATE nav_rankings SET aum_cr=? WHERE nav_date=? AND fund_label=?", (aum_cr, date_str, label))
                    updated += 1
                else:
                    conn.execute("INSERT OR IGNORE INTO nav_rankings (nav_date, fund_label, aum_cr) VALUES (?,?,?)", (date_str, label, aum_cr))
                    inserted += 1
            conn.commit()
            print(f"  {amfi_str}: {len(aum_map):2d} funds  [updated={updated} inserted={inserted} empty={empty}]")
            d += timedelta(days=1); _time2.sleep(0.35)
    conn.close()
    print(f"\n  Done ARB AUM: updated={updated} inserted={inserted} empty={empty}")


if __name__ == "__main__":
    print("=" * 50)
    print("  ARB FUND — Full Backfill")
    print("=" * 50)
