...
# AUM BACKFILL — inlined (liquid portion only, uses dailyAUM)
# ══════════════════════════════════════════════════════════════════════════════
...
_AUM_COOKIES = {"__gsas": "ID=f8678d6ec5abaf6c:T=1764739479:RT=1764739479:S=ALNI_MZB7yKBaAgnLq05icnN8Pcx2bKEHA"}
_AUM_PAYLOAD = {"maturityType": 1, "category": 2, "subCategory": 20, "mfid": 0}

_LIQUID_AUM_MAP = {
    "360 ONE": "360 ONE", "ABAKKUS": "Abakkus", "ADITYA BIRLA": "Aditya Birla Sun Life",
    "AXIS LIQUID": "Axis", "BAJAJ FINSERV": "Bajaj Finserv", "BANDHAN": "Bandhan",
    "BANK OF INDIA": "Bank of India", "BARODA BNP": "Baroda BNP Paribas",
    "CANARA ROBECO": "Canara Robeco", "CAPITALMIND": "Capitalmind",
    "DSP LIQUID": "DSP", "DSP LIQUIDITY": "DSP", "EDELWEISS": "Edelweiss",
    "FRANKLIN": "Franklin Templeton", "GROWW": "Groww", "HDFC LIQUID": "HDFC",
    "HSBC LIQUID": "HSBC", "ICICI PRUDENTIAL": "ICICI P", "INVESCO": "Invesco",
    "ITI LIQUID": "ITI", "JIOBLACKROCK": "Jio B", "JIO BLACKROCK": "Jio B",
    "JM LIQUID": "JM Financial", "KOTAK LIQUID": "Kotak Mahindra", "LIC MF": "LIC",
    "MAHINDRA MANULIFE": "Mahindra Manulife", "MIRAE ASSET": "Mirae Asset",
    "MOTILAL OSWAL": "Motilal Oswal", "NAVI LIQUID": "Navi",
    "NIPPON INDIA LIQUID": "Nippon India", "PGIM INDIA": "PGIM India",
    "PARAG PARIKH": "PPFAS", "QUANTUM LIQUID": "Quantum", "QUANT LIQUID": "quant",
    "SBI LIQUID": "SBI", "SHRIRAM": "Shriram", "SUNDARAM LIQUID": "Sundaram",
    "TATA LIQUID": "Tata", "THE WEALTH COMPANY": "The Wealth Company",
    "TRUSTMF": "Trust", "UNIFI": "Unifi", "UNION LIQUID": "Union", "UTI": "UTI",
    "WHITEOAK CAPITAL": "WhiteOak Capital",
}

def _aum_map_name(raw):
    u = raw.upper()
    for key, label in _LIQUID_AUM_MAP.items():
        if key in u:
            return label
    return None
...
def _backfill_liquid_aum():
    from datetime import date, timedelta
    print(f"\n{'='*60}")
    print(f"  LIQUID AUM BACKFILL — {_AUM_START} to {date.today()}")
    print(f"{'='*60}")
    conn = sqlite3.connect(DB_PATH)
    ...
    conn.close()
    print(f"\n  Done LIQUID AUM: updated={updated} inserted={inserted} empty={empty}")


if __name__ == "__main__":
    print("=" * 50)
    print("  LIQUID FUND — Full Backfill")
    print("=" * 50)
    ...
    _backfill_liquid_aum()
    print("\n" + "=" * 50)
    print("  LIQUID BACKFILL COMPLETE")
    print("=" * 50)
